...
class ViTransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, num_patches + 1, dim))  # type: ignore
        self.patch_to_embedding = nn.Linear(patch_dim, dim)
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))  # type: ignore
    ...
    def forward(self, img, **kwargs):
        ...
        pos_emb_ind = repeat(torch.arange(h)*(self.max_width//p-w), 'h -> (h w)', w=w)+torch.arange(h*w) # type: ignore
